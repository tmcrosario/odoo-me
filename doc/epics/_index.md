...
| EPIC-004 | Reacotamiento de la carga de ME: traspaso de clasificación a JUNCO | `me` (+ `junco`) | Draft | L/sensible. ME cede jurisdiction/source a JUNCO **solo en DEM** (CM no cede, TMC intacto); conserva subject/specification/reference; rediseña 1er movimiento DEM. Contrato joint cerrado (D-1/D-2/D-3/D-4 ↔ junco EPIC-010): incluye método `action_set_origin_from_junco` (write-guard). En `/product-spec`→`/contract-draft` |
