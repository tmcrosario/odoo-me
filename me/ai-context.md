...
| `fojas` | No | Integer, default=0; snapshot of page count at the time of the movement; auto-populated in automatic movements; pre-loaded via default_get() for manual movements from the **last movement's fojas** (not from expediente.fojas, which only reflects the creation value) |
...
| `user_id` | No | Odoo user responsible for the expediente at the destination of this movement (NOT who loaded it — that is `create_uid`); default=current user; auto-filled via `@api.onchange('destination_dependence_id')` from `tmc.dependence.default_responsible_id` when destination is internal (#030); cleared when destination is external; readonly in view when `is_automatic=True` |
